...
class city_dset(BaseDataset):
    def __init__(self, data_root, data_list, trs_form, seed, n_sup, split="val", acp=False, paste_trs=None, prob=0.5, flag=None):
    ...
    def __getitem__(self, index):  
        ...
        if self.flag == 'unlabeled':  
            ...
            image, label = crop(image, label, 769, ignore_value)  
...
def build_transfrom(cfg, acp=False):  
    ...
    trs_form.append(psp_trsform.ToTensor())  # FloatTensor of shape (1 x C x H x W)
...
def build_city_semi_loader(split, all_cfg, seed=0):  # split train
    cfg_dset = all_cfg["dataset"]  #
    ...
    workers = cfg.get("workers", 2)  # 2
    batch_size = cfg.get("batch_size", 1)  # 2
    ...
    dset = city_dset(cfg["data_root"], cfg["data_list"], trs_form, seed, n_sup, split, acp=acp, paste_trs=paste_trs,
                     prob=prob)  
