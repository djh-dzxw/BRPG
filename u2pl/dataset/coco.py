...
class coco_dset(BaseDataset):
    def __init__(
        self, data_root, data_list, trs_form, seed=0, n_sup=118287, split="val",
            acp=False,
            paste_trs=None,
            prob=0.5,
            flag=None
    ):
        super(coco_dset, self).__init__(data_list)
...
def build_cocoloader(split, all_cfg, seed=0):
    ...
    n_sup = cfg.get("n_sup", 118287)
    # build transform
    trs_form = build_transfrom(cfg)  
    dset = coco_dset(cfg["data_root"], cfg["data_list"], trs_form, seed, n_sup)
...
def build_coco_semi_loader(split, all_cfg, seed=0):
    cfg_dset = all_cfg["dataset"]
    ...
    workers = cfg.get("workers", 2)
    batch_size = cfg.get("batch_size", 1)
    n_sup = 118287 - cfg.get("n_sup", 118287)
    ...
    dset = coco_dset(cfg["data_root"], cfg["data_list"], trs_form, seed, n_sup, split, acp=acp,
        paste_trs=paste_trs,
        prob=prob)
