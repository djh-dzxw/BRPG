...
class dec_deeplabv3_plus(nn.Module):
    def __init__(
        ...
        self.net.append(self.low_conv)
        self.net.append(self.aspp)
        self.net.append(self.head)
        self.net.append(self.classifier)
        ...
        if self.rep_head:  
            ...
            self.net.append(self.representation)
    ...
    def init_params_decoder(self):  
        for net in self.net:  
            for feature in net.modules():  
                if isinstance(feature, (nn.Conv1d, nn.Conv2d, nn.Conv3d)):
                    nn.init.kaiming_normal_(feature.weight, mode='fan_in', nonlinearity='relu')
                elif isinstance(feature, self.norm_layer):
                    feature.eps = 1e-5
                    feature.momentum = 0.1
                    nn.init.constant_(feature.weight, 1)
                    nn.init.constant_(feature.bias, 0)
