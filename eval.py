...
def main():  
    ...
    if "cityscapes" in cfg["dataset"]["type"]:  # city
        validate_city(
            model,
            num_classes,
            data_list,
    ...
    elif 'coco' in cfg["dataset"]["type"]:  # coco
        validate_coco(
            model,
            num_classes,
            data_list,
...
def validate_city(
    ...
    for i, (input_pth, label_path) in enumerate(data_list):  
        ...
        image = torch.Tensor(image).permute(2, 0, 1)  # chw
        image = image.contiguous().unsqueeze(dim=0)  # 1 c h w
        h, w = image.size()[-2:]  
...
def validate_coco(
    ...
    for i, (input_pth, _) in enumerate(data_list):
        ...
        image = image.contiguous().unsqueeze(dim=0)  # bchw
        h, w = image.size()[-2:]
        prediction = torch.zeros((classes, h, w), dtype=torch.float).cuda()
        for scale in scales:  
            new_h = round(h * scale)
